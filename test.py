...
from MARL.MultiAgent import IMARL, DepMARL
from sac import SAC, SMAAC
from SACD import SacdGoal, SacdSimple
from DQN import DQN, DQN2
from PPO import PPO
...
DATA_SPLIT = {
    ...
    '14': (list(range(0, 40*26, 40)), list(range(1, 100*10+1, 100)), list(range(2, 100*10+2, 100))),
...
def cli():
    ...
    parser.add_argument('-la', '--load_agent', type=str, default='',
    ...
    parser.add_argument('-ns', '--nb_steps', type=int, default=100,
                        help='the total number of agent steps')
    parser.add_argument('-ev', '--eval_steps', type=int, default=50,
    ...
    parser.add_argument('-mr', '--max_reward', type=int, default=10,
                        help='max reward during training')
    parser.add_argument('-i', '--input', type=str, nargs='+', default=['p_i', 'r', 'o', 'd', 'm'],
                        help='Options for input information in the GNN.\n '
                             ...
                             ' "d" - danger, \n'
                             ' "m" - maintenance')
    parser.add_argument('-fc', '--forecast', type=int, default=0,
                        help='forecast as input for training')
    ...
    # Middle level agent
    parser.add_argument('-ma', '--middle_agent', type=str, default="capa", choices=['fixed', 'random', 'capa'])

    # Choose the lowest level agent
    parser.add_argument("-a", "--agent", type=str, default="dppo",
                        choices=['sac', 'sac2', 'sacd', 'sacd2',
                                 'masacd', 'dsacd', 'dqn', 'dqn2',
                                 'ppo', 'mappo', 'dppo'])
    ...
    parser.add_argument('-u', '--update_start', type=int, default=2,
    ...
    parser.add_argument('-te', '--target_entropy_scale', type=float, default=0.98,
                        help="coefficient for scaling the autotune entropy target")

    # PPO parameters
    parser.add_argument('-ep', '--epsilon', type=float, default=0.2,
...
def def_env(args):
    return grid2op.make(ENV_CASE[args.case],
...
def select_chronics(env_path, env, test_env, case, eval=False):
    ...
    train_chron_str = "|".join(['%02d' % chr if case == '5' else '%04d' % chr
                                for chr in train_chronics])
    ...
    valid_chron_str = "|".join(['%02d' % chr if case == '5' else '%04d' % chr
                                for chr in valid_chronics])
...
def load_agent(args, my_dir, my_agent):
    ...
    with open(os.path.join(load_model_dir, 'param.json'), 'rt') as f:
        ...
        if params['case'] != args.case:
            raise Exception(f"Model of agent loaded is for"
                            f" case {params['case']}, but current agent is for case {args.case}")
        if params['forecast'] != args.forecast:
            raise Exception(f"Model of agent loaded contains"
...
if __name__ == '__main__':
    ...
    trainer = TRAINER(my_agent, env, test_env, dn_ffw, ep_infos, max_reward=args.max_reward, rw_func=args.reward)

    tic = datetime.now()
    trainer.train(
        args.seed, args.nb_steps, args.eval_steps,
        train_chronics, valid_chronics, output_result_dir, model_path, get_max_ffw(args.case),
