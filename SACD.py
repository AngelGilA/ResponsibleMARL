...
from my_converters import DiscActionConverter
...
class SacdSimple(SingleAgent):
    ...
    def update(self):
        if len(self.memory) >= self.last_mem_len + self.update_freq:
            ...
            # critic loss
            Q1_loss, Q2_loss = self.get_critic_loss(stacked_x, stacked_t, adj, actions, rewards,
    ...
    def get_critic_loss(self, stacked_x, stacked_t, adj, actions, rewards,
        ...
        with torch.no_grad():
            ...
            action_probs, log_action_probs = \
                self.produce_action(actor_input2, adj2, learn=True)
...
class SacdGoal(SacdSimple):
    ...
    def create_action_converter(self, env, mask, mask_hi, **kwargs):
        ...
        return DiscActionConverter(env, mask, mask_hi)
