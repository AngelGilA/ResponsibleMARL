...
from test import make_envs, select_chronics, make_dirs, get_max_ffw, make_agent, seed_everything
...
def cli():
    ...
    parser.add_argument('-s', '--seeds', type=int, default=2,
    ...
    parser.add_argument('-ns', '--nb_steps', type=int, default=200,
                        help='the total number of agent steps')

    parser.add_argument('-ev', '--eval_steps', type=int, default=25,
    ...
    parser.add_argument("-a", "--agent", type=str, default="dsacd",
    ...
    parser.add_argument('-tr', '--n_trials', type=int, default=4,
                        help='Maximum number of trials for param tuning')
    parser.add_argument('-nj', '--n_jobs', type=int, default=1,
                        help='Number of jobs to run in parallel')
    parser.add_argument('-st', '--n_startup_trials', type=int, default=3,
...
def trial_one_seed(env, env_path, kwargs, test_env, dn_ffw, ep_infos, seed,
                   train_chronics, valid_chronics, output_result_dir, model_path, trial, best_score):
    ...
    # Train the model
    best_score = trainer.train(seed, nb_steps, eval_steps,
                               train_chronics, valid_chronics, output_result_dir,
                               model_path, get_max_ffw(case), best_score=best_score, verbose=False)
...
def objective(trial: optuna.Trial) -> float:
    ...
    try:
        for seed in seeds:
            last_mean_reward[seed], best_score = trial_one_seed(env, env_path, kwargs, test_env, dn_ffw, ep_infos, seed,
                                                                train_chronics, valid_chronics, output_result_dir,
                                                                model_path, trial, best_score)
...
def save_optuna_visualizations(output_dir):
    from optuna.visualization.matplotlib import plot_optimization_history, plot_param_importances, \
        plot_parallel_coordinate, plot_timeline, plot_intermediate_values, plot_contour
    try:
...
if __name__ == '__main__':
    ...
    if args.load_study:
        ...
        try:
            study_id = storage.create_new_study([optuna.study.StudyDirection.MAXIMIZE],
